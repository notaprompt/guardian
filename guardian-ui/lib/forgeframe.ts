...
type ProviderType = 'claude-cli' | 'openai-compatible' | 'ollama';
...
let _allModels: Model[] = [...DEFAULT_MODELS];
...
const _PROVIDER_COST_ORDER: Record<string, number> = {
  'openai-compatible': 0,
  ollama: 0,
  'claude-cli': 1,
};
...
function resolveModel(message: string, override: string | null): ResolvedModel {
  ...
  const settings = readJSON<ForgeFrameSettings>(FILES.settings, {});
