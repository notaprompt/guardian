...
type Dimension = 'emotional' | 'professional' | 'cognitive' | 'relational'
...
const DIMENSION_PATTERNS: Record<Dimension, RegExp[]> = {} as any;
for (const [dim, keywords] of Object.entries(DIMENSION_KEYWORDS)) {
  DIMENSION_PATTERNS[dim as Dimension] = keywords.map((kw) => new RegExp(`\\b${kw}\\b`, 'gi'));
}
 ...
 */
function classifyText(text: string): Record<Dimension, number> {
  const result = {} as Record<Dimension, number>;
  if (!text || typeof text !== 'string') {
    for (const dim of Object.keys(DIMENSION_KEYWORDS) as Dimension[]) result[dim] = 0;
    return result;
  }

  const lower = text.toLowerCase();

  for (const [dim, patterns] of Object.entries(DIMENSION_PATTERNS) as [Dimension, RegExp[]][]) {
    let count = 0;
 ...
 */
function computeDimensionScores(db: GuardianDb, days: number = 30): DimensionScoresResult {
  const now = new Date().toISOString();
  const cutoff = new Date(Date.now() - days * 86400000).toISOString();
  const prevCutoff = new Date(Date.now() - days * 2 * 86400000).toISOString();

  const rawDb = db.db();
  const dimensions = Object.keys(DIMENSION_KEYWORDS) as Dimension[];

  // Initialize accumulators
  const scores = {} as Record<Dimension, number>;
  const reframeCounts = {} as Record<Dimension, number>;
  const entityCounts = {} as Record<Dimension, number>;
  ...
  try {
    const reframeRows = rawDb.prepare(
      'SELECT reframe_text, identity_dimension FROM reframe_events WHERE created_at >= ?'
    ).all(cutoff) as { reframe_text: string; identity_dimension: string }[];

    // Count directly from identity_dimension column
    for (const row of reframeRows) {
      if (row.identity_dimension && scores[row.identity_dimension as Dimension] !== undefined) {
        reframeCounts[row.identity_dimension as Dimension]++;
      }
    }
  ...
  try {
    const compressionRows = rawDb.prepare(
      'SELECT content FROM compression_levels WHERE level >= 2 AND created_at >= ?'
  ...
  try {
    const noteRows = rawDb.prepare(
      'SELECT content FROM notes WHERE updated_at >= ?'
    ).all(cutoff) as { content: string }[];
  ...
  try {
    const queueRows = rawDb.prepare(
      'SELECT text FROM queue_items WHERE created_at >= ?'
    ).all(cutoff) as { text: string }[];
  ...
  const trends = {} as Record<Dimension, Trend>;
  for (const dim of dimensions) {
  ...
  let dominantDimension: Dimension = dimensions[0];
  let neglectedDimension: Dimension = dimensions[0];
  let maxDim = -1;
  let minDim = Infinity;
  ...
  let neglectedNonZero: Dimension | null = null;
  for (const dim of dimensions) {
  ...
  if (neglectedNonZero) neglectedDimension = neglectedNonZero;
 ...
 */
function computeRawScores(rawDb: any, startDate: string, endDate: string, dimensions: Dimension[]): Record<Dimension, number> {
  const scores = {} as Record<Dimension, number>;
  ...
  try {
    const rows = rawDb.prepare(
      'SELECT reframe_text FROM reframe_events WHERE created_at >= ? AND created_at < ?'
    ).all(startDate, endDate) as { reframe_text: string }[];
    const text = rows.map((r) => r.reframe_text || '').join(' ');
    const hits = classifyText(text);
    for (const dim of dimensions) scores[dim] += hits[dim] * SOURCE_WEIGHTS.reframes;
  } catch (_) {}
  ...
  try {
    ...
    `).all(startDate, endDate) as { name: string }[];
    const text = rows.map((r) => r.name || '').join(' ');
    const hits = classifyText(text);
    for (const dim of dimensions) scores[dim] += hits[dim] * SOURCE_WEIGHTS.knowledgeGraph;
  } catch (_) {}

  // Compression
  try {
    const rows = rawDb.prepare(
      'SELECT content FROM compression_levels WHERE level >= 2 AND created_at >= ? AND created_at < ?'
    ).all(startDate, endDate) as { content: string }[];
    const text = rows.map((r) => r.content || '').join(' ');
    const hits = classifyText(text);
    for (const dim of dimensions) scores[dim] += hits[dim] * SOURCE_WEIGHTS.compression;
  } catch (_) {}

  // Notes
  try {
    const rows = rawDb.prepare(
      'SELECT content FROM notes WHERE updated_at >= ? AND updated_at < ?'
    ).all(startDate, endDate) as { content: string }[];
    const text = rows.map((r) => r.content || '').join(' ');
    const hits = classifyText(text);
    for (const dim of dimensions) scores[dim] += hits[dim] * SOURCE_WEIGHTS.notes;
  } catch (_) {}

  // Queue
  try {
    const rows = rawDb.prepare(
      'SELECT text FROM queue_items WHERE created_at >= ? AND created_at < ?'
    ).all(startDate, endDate) as { text: string }[];
    const text = rows.map((r) => r.text || '').join(' ');
    const hits = classifyText(text);
    for (const dim of dimensions) scores[dim] += hits[dim] * SOURCE_WEIGHTS.queue;
  } catch (_) {}
 ...
 */
function computeDimensionTimeline(db: GuardianDb, weeks: number = 12): TimelineEntry[] {
  const rawDb = db.db();
  const dimensions = Object.keys(DIMENSION_KEYWORDS) as Dimension[];
