 ...
 * @returns {{ [dimension]: number }} hit counts per dimension
 */
function classifyText(text) {
  if (!text || typeof text !== 'string') {
    const result = {};
    for (const dim of Object.keys(DIMENSION_KEYWORDS)) result[dim] = 0;
    return result;
  }
 ...
 *
 * @param {Object} db   - database module (with .db() for raw SQLite)
 * @param {number} days - time window in days (default 30)
 * @returns {Object} dimension scores, trends, dominant/neglected
 */
function computeDimensionScores(db, days = 30) {
  ...
  try {
    // Get entities from sessions within the window
  ...
  for (const dim of dimensions) {
    ...
    // For neglected: find lowest non-zero, or lowest overall
 ...
 * @returns {Array<{ week: string, dimensions: Object }>}
 */
